  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        ...
        setError('Credenciales incorrectas. Intenta con los usuarios de prueba.');
      }
    } catch (err: any) {
        ...
        <div className="p-8">
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-slate-600/50 focus:ring-4 focus:ring-red-100 dark:focus:ring-red-500/30 focus:border-red-500 outline-none transition-all bg-gray-50 dark:bg-slate-700/50 dark:text-white focus:bg-white dark:focus:bg-slate-700 placeholder-gray-400 dark:placeholder-slate-500 shadow-sm" placeholder="Ej. Admin" required disabled={loading} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">PIN de Acceso</label>
              <input type="password" value={pin} onChange={(e) => setPin(e.target.value)} className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-slate-600/50 focus:ring-4 focus:ring-red-100 dark:focus:ring-red-500/30 focus:border-red-500 outline-none transition-all bg-gray-50 dark:bg-slate-700/50 dark:text-white focus:bg-white dark:focus:bg-slate-700 placeholder-gray-400 dark:placeholder-slate-500 shadow-sm" placeholder="••••" required disabled={loading} />
            </div>
            {error && (<div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-3 rounded-xl text-sm text-center font-medium animate-pulse border border-red-100 dark:border-red-900/30">{error}</div>)}
            <button type="submit" disabled={loading} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-button-red active:scale-[0.98] flex items-center justify-center gap-2 group">
              {loading ? <Loader2 className="animate-spin text-white" /> : 'Iniciar Sesión'}
            </button>
          </form>
          <div className="mt-8 text-center text-xs text-gray-400 dark:text-slate-500">Sistema de Gestión v2.0</div>
        </div>
      </div>
    </div>
  );
