    ...
    <div className="p-4 md:p-8 bg-gray-50 dark:bg-slate-900 min-h-full pb-24 md:pb-6 font-sans transition-colors duration-300">
      ...
      <div className="mb-8">
        <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white flex items-center gap-3">
          <span className="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 p-2 rounded-xl"><FileText size={32} /></span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {orders.map((order) => (
          <div key={order.batchId} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700/50 p-6 hover:shadow-xl dark:hover:shadow-none dark:hover:border-slate-600 transition-all cursor-pointer group" onClick={() => setSelectedOrder(order)}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <h3 className="font-bold text-gray-800 dark:text-slate-200 mt-2 text-lg group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors">{order.batchId}</h3>
              </div>
            ...
            <div className="flex items-center gap-3 border-t border-gray-100 dark:border-slate-700/50 pt-4">
              <button 
                ...
                className="flex-1 bg-gray-900 dark:bg-slate-700 text-white text-sm font-bold py-3 rounded-xl hover:bg-red-600 dark:hover:bg-red-500 transition-colors flex items-center justify-center gap-2 shadow-sm"
              >
              ...
              {currentUser.role === UserRole.ADMIN && (
                <button 
                  ...
                  className="p-3 text-gray-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors border border-gray-100 dark:border-slate-700/50 hover:border-red-100"
        ...
        {orders.length === 0 && (
          <div className="col-span-full py-20 text-center text-gray-400 dark:text-slate-600 bg-white dark:bg-slate-800 rounded-3xl border-2 border-dashed border-gray-200 dark:border-slate-700/50">
            <div className="bg-gray-50 dark:bg-slate-700/30 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4">
              <FileText size={40} className="opacity-40" />
            </div>
            <p className="text-xl font-bold text-gray-500 dark:text-slate-500">No hay pedidos registrados</p>
            <p className="text-sm mt-1">Realiza un nuevo pedido para verlo aquí.</p>
          </div>
        )}
      </div>
      ...
      {selectedOrder && (
        ...
        >
            ...
            <div className="flex gap-2 ml-auto w-full md:w-auto">
              {currentUser.role === UserRole.ADMIN && (
                <button 
                  ...
                  className="flex-1 md:flex-none bg-red-600/20 text-red-200 border border-red-500/50 px-4 py-3 rounded-xl font-bold backdrop-blur-md flex items-center justify-center gap-2 hover:bg-red-600 hover:text-white transition-all"
              ...
              <button 
                onClick={handlePrint} 
                className="flex-1 md:flex-none bg-blue-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 hover:bg-blue-500 transition-all"
              >
          ...
          >
            ...
            <button 
              onClick={() => setSelectedOrder(null)}
              className="absolute top-4 right-4 p-2 bg-gray-100 rounded-full text-gray-500 hover:bg-gray-200 md:hidden no-print"
            >
              <X size={24} />
            </button>
