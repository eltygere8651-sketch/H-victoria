...
export const OrderPdfDocument: React.FC<OrderPdfDocumentProps> = ({ order }) => {
  return (
    <div className="bg-white text-black p-12 font-sans" style={{ width: '210mm', minHeight: '297mm', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div className="flex justify-between items-start border-b-2 border-black pb-8 mb-8">
        <div className="flex items-center gap-4">
          <Logo size="lg" solid />
          <div className="mt-2">
            <h1 className="text-3xl font-black uppercase tracking-tighter text-black">Hub</h1>
            <p className="text-red-600 font-bold uppercase tracking-[0.3em] text-sm">Reporte de Pedido</p>
          </div>
        </div>
        <div className="text-right">
          <h2 className="text-2xl font-mono font-bold text-black">#{order.batchId}</h2>
      ...
      <div className="grid grid-cols-2 gap-8 mb-10">
        <p>
          ...
          <span className="text-2xl font-extrabold text-black">{order.departmentName}</span>
        </p>
        <p>
          <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Solicitante</span><br />
          <span className="text-2xl font-extrabold text-black">{order.requestedBy}</span>
        </p>
      </div>
          ...
          <tbody>
            {order.items.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-200">
                <td className="py-4 font-bold text-lg text-black">{item.productName}</td>
                <td className="py-4 text-center font-black text-xl text-black">{item.quantity}</td>
                <td className="py-4 text-right text-gray-500 font-semibold">{item.unit || 'uds.'}</td>
                <td className="py-4 text-right">
                  <div className="w-8 h-8 border-2 border-gray-300 rounded-lg inline-block"></div>
                </td>
              </tr>
            ))}
          </tbody>
