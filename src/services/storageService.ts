...
export const registerCatalogFromImage = async () => {
  const sessionUser = getSession();
  const isAdmin = sessionUser?.role === UserRole.ADMIN;
  
  // Determinamos si el sistema está inicializado
  let isInitialized = false;
  try {
    const initDoc = await db.collection(KEYS.SYSTEM).doc('initialization').get();
    isInitialized = initDoc.exists && initDoc.data()?.isInitialized;
  } catch (e) {
    // Si no podemos leer la inicialización, procedemos si no hay sesión admin activa (bootstrap)
  }

  // Solo intentamos sincronizar si somos admin O si el sistema NO está inicializado
  if (!isAdmin && isInitialized) {
     return;
  }

  console.log("Iniciando sincronización masiva del catálogo desde imagen...");
  
  try {
    const productsSnapshot = await db.collection(KEYS.PRODUCTS).get();
    const existingProductsMap = new Map(productsSnapshot.docs.map(doc => [doc.data().name.toLowerCase().trim(), { id: doc.id, ...doc.data() }]));
    let count = 0;
    let updateCount = 0;

    const fullCatalog = [
      // --- CAFETERÍA ---
      { name: 'Leche', max: 42, unit: 'Bot', cat: 'Cafetería' },
      { name: 'Leches Varias', max: 3, unit: 'Bot', cat: 'Cafetería' },
      { name: 'Azucarillos', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Azucar Morena', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Sacarina', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Café', max: 12, unit: 'Kilos', cat: 'Cafetería' },
      { name: 'Descafeinado', max: 4, unit: 'Kilos', cat: 'Cafetería' },
      { name: 'Cola cao / Nesquik', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Descafeinado sobre', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Manzanilla', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Poleo/Te/Tila/Te verde', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Infusiones varias', max: 1, unit: 'Caja', cat: 'Cafetería' },
      { name: 'Vasos y tapas Llevar', max: 20, unit: 'Unid', cat: 'Suministros' },

      // --- REFRESCOS / AGUAS ---
      { name: 'Coca Cola 350 cl', max: 35, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Coca Cola', max: 35, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Coca Cola Zero 350 cl', max: 35, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Coca Cola Zero', max: 12, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Cola Cola Zero-Zero', max: 12, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Kas Limón', max: 28, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Kas Naranja', max: 28, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Bitter Kas', max: 14, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Tónica Scheweeps', max: 35, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Tónica Rosa', max: 14, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Sprite', max: 14, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Red bull', max: 12, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Nestea', max: 21, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Aquarius Limón', max: 21, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Aquarius Naranja', max: 21, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Mosto', max: 8, unit: 'Bot', cat: 'Refrescos' },
      { name: 'Zumo Piña', max: 2, unit: 'Bot', cat: 'Zumos' },
      { name: 'Zumo Naranja', max: 4, unit: 'Bot', cat: 'Zumos' },
      { name: 'Zumo Melocotón', max: 2, unit: 'Bot', cat: 'Zumos' },
      { name: 'Batido chocolate', max: 12, unit: 'Bot', cat: 'Batidos' },
      { name: 'Agua 0.33 Tetra brik', max: 40, unit: 'Bot', cat: 'Agua' },
      { name: 'Agua con gas', max: 12, unit: 'Bot', cat: 'Agua' },
      { name: 'Agua 1.5', max: 24, unit: 'Bot', cat: 'Agua' },
      { name: 'Gaseosa', max: 12, unit: 'Bot', cat: 'Agua' },
      { name: 'Agua ½ Cristal', max: 12, unit: 'Bot', cat: 'Agua' },

      // --- CERVEZAS ---
      { name: 'Estrella Galicia', max: 24, unit: 'Bot', cat: 'Cervezas' },
      { name: '18/70', max: 18, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Mahou-CELIACOS', max: 12, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Mahou 1/5', max: 30, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Mahou 1/3', max: 24, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Alhambra', max: 18, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Maestra', max: 18, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Mixta', max: 12, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Sin TOSTADA', max: 12, unit: 'Bot', cat: 'Cervezas' },
      { name: 'Sin alcohol', max: 18, unit: 'Bot', cat: 'Cervezas' },
      { name: '0\'0 Manzana', max: 12, unit: 'Bot', cat: 'Cervezas' },

      // --- VINOS ---
      { name: 'Blanco', max: 4, unit: 'Bot', cat: 'Vinos' },
      { name: 'Blanco Cantoblanco', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Blanco FRISANTE', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Blanco VERDEJO', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Rosado VARIOS', max: 2, unit: 'Bot', cat: 'Vinos' },
      { name: 'Rosado Cantoblanco', max: 2, unit: 'Bot', cat: 'Vinos' },
      { name: 'Tinto Joven FAUSTI', max: 8, unit: 'Bot', cat: 'Vinos' },
      { name: 'Tinto Joven MOCETE', max: 8, unit: 'Bot', cat: 'Vinos' },
      { name: 'Crianza', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Crianza Faustino', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Crianza Azpilicueta', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Crianza Pierola', max: 5, unit: 'Bot', cat: 'Vinos' },
      { name: 'Crianza Ramón Bilbao', max: 5, unit: 'Bot', cat: 'Vinos' },

      // --- LICORES / VERMUT ---
      { name: 'Martini Rojo', max: 3, unit: 'Bot', cat: 'Vermut' },
      { name: 'Martini Blanco', max: 3, unit: 'Bot', cat: 'Vermut' },
      { name: 'Vermut CUACHO', max: 3, unit: 'Bot', cat: 'Vermut' },
      { name: 'Moscatel', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Magno / Soberano', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Anis / Ponche', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Licor 43 / Countreau', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Pacharán la navarra', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Pacharán Zoco / Baines', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Baileys', max: 3, unit: 'Bot', cat: 'Licores' },
      { name: 'Crema orujo', max: 3, unit: 'Bot', cat: 'Licores' },
      { name: 'Orujo blanco', max: 1, unit: 'Bot', cat: 'Licores' },
      { name: 'Licor hierbas', max: 3, unit: 'Bot', cat: 'Licores' },
      { name: 'Licor Manzana / Melocotón', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Licor Avellana / Frangelico', max: 2, unit: 'Bot', cat: 'Licores' },
      { name: 'Peche / Tequila', max: 2, unit: 'Bot', cat: 'Licores' },

      // --- COMBINADOS (WHISKY, GIN, RON, VODKA) ---
      { name: 'White label', max: 3, unit: 'Bot', cat: 'Whisky' },
      { name: 'J.B / J.B 15 Años', max: 2, unit: 'Bot', cat: 'Whisky' },
      { name: 'Ballantines/Balla 12 Año', max: 2, unit: 'Bot', cat: 'Whisky' },
      { name: 'Chivas/ Cardhu', max: 2, unit: 'Bot', cat: 'Whisky' },
      { name: 'Tanqueray / 0\'0', max: 3, unit: 'Bot', cat: 'Ginebra' },
      { name: 'Seagrans', max: 3, unit: 'Bot', cat: 'Ginebra' },
      { name: 'Beefeater', max: 3, unit: 'Bot', cat: 'Ginebra' },
      { name: 'Gordons // Rosa', max: 3, unit: 'Bot', cat: 'Ginebra' },
      { name: 'Bombay/Bomb.Saphire', max: 3, unit: 'Bot', cat: 'Ginebra' },
      { name: 'Smirnof', max: 3, unit: 'Bot', cat: 'Vodka' },
      { name: 'Absolut', max: 3, unit: 'Bot', cat: 'Vodka' },
      { name: 'Brugal', max: 3, unit: 'Bot', cat: 'Ron' },
      { name: 'Barcelo / Cacique', max: 3, unit: 'Bot', cat: 'Ron' },
      { name: 'Habana 5/Habana 3', max: 3, unit: 'Bot', cat: 'Ron' },
      { name: 'Matusalén', max: 3, unit: 'Bot', cat: 'Ron' },
      { name: 'Bacardi blanco', max: 2, unit: 'Bot', cat: 'Ron' },

      // --- COMIDA / APERITIVOS ---
      { name: 'Patatas fritas', max: 1, unit: 'Caja', cat: 'Aperitivos' },
      { name: 'Limones', max: 1, unit: 'Barq', cat: 'Frescos' },
      { name: 'Naranja', max: 1, unit: 'Barq', cat: 'Frescos' },
      { name: 'Yogures', max: 24, unit: 'Unid', cat: 'Lácteos' },
      { name: 'Mermeladas/nocilla', max: 1, unit: 'Caja', cat: 'Comida' },
      { name: 'Tomate / sal', max: 1, unit: 'Caja', cat: 'Comida' },
      { name: 'Aceite / vinagre', max: 1, unit: 'Caja', cat: 'Comida' },
      { name: 'Mahonesa / ketchup', max: 1, unit: 'Caja', cat: 'Comida' },
      { name: 'Frutos secos', max: 1, unit: 'Caja', cat: 'Aperitivos' },
      { name: 'Madalenas', max: 1, unit: 'Caja', cat: 'Desayunos' },
      { name: 'Productos celiacos', max: 1, unit: 'Caja', cat: 'Especial' },
      { name: 'Cereales // Panes', max: 1, unit: 'Caja', cat: 'Comida' },

      // --- SUMINISTROS ---
      { name: 'Servilletas desayunos', max: 1, unit: 'Caja', cat: 'Suministros' },
      { name: 'Servilleta aperitivo', max: 1, unit: 'Caja', cat: 'Suministros' },
      { name: 'Servilleta servilletero', max: 1, unit: 'Caja', cat: 'Suministros' },
      { name: 'Palillos', max: 1, unit: 'Caja', cat: 'Suministros' },
      { name: 'Bolsa cubo blanca', max: 1, unit: 'Rollo', cat: 'Limpieza' },
      { name: 'Bolsa cubo negra', max: 1, unit: 'Rollo', cat: 'Limpieza' },
      { name: 'Bolsa cubo amarilla', max: 1, unit: 'Rollo', cat: 'Limpieza' },
      { name: 'Ambientador', max: 6, unit: 'Bot', cat: 'Limpieza' },
    ];
    ...
    for (const p of fullCatalog) {
      const existing = existingProductsMap.get(p.name.toLowerCase().trim());
      
      if (!existing) {
        const docRef = db.collection(KEYS.PRODUCTS).doc();
        batch.set(docRef, {
          id: docRef.id,
          name: p.name,
          category: p.cat,
          quantity: 0,
          unit: p.unit,
          minThreshold: Math.floor(p.max / 4) || 1, // Umbral del 25% para alerta normal
          maxThreshold: p.max,
          departmentId: 'd-bar',
          departmentName: 'Bar / Cafetería',
          departmentIds: ['d-bar'],
          departmentNames: ['Bar / Cafetería'],
          updatedAt: Date.now()
        });
        count++;
        batchCount++;
        ...
        const currentDeptIds = existingData.departmentIds || (existingData.departmentId ? [existingData.departmentId] : []);
        // Force update if maxThreshold, unit OR category is wrong OR d-bar is missing
        const needsUpdate = existingData.maxThreshold !== p.max || 
                            existingData.unit !== p.unit ||
                            existingData.category !== p.cat ||
                            !currentDeptIds.includes('d-bar');
        
        if (needsUpdate) {
          const docRef = db.collection(KEYS.PRODUCTS).doc(existingData.id);
          batch.update(docRef, {
            maxThreshold: p.max,
            unit: p.unit,
            category: p.cat,
            departmentIds: Array.from(new Set([...currentDeptIds, 'd-bar'])),
            departmentNames: Array.from(new Set([...(existingData.departmentNames || []), 'Bar / Cafetería'])),
            updatedAt: Date.now()
          });
          updateCount++;
          batchCount++;
    ...
    // Marcar como inicializado si no lo estaba
    await db.collection(KEYS.SYSTEM).doc('initialization').set({
      isInitialized: true,
      lastCatalogSync: Date.now()
    }, { merge: true });

    return { created: count, updated: updateCount };
  } catch (error) {
