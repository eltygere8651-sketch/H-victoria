  ...
  if (user.role === UserRole.ADMIN && auth.currentUser) {
    ...
    try {
      ...
      if (!e.message?.includes('permissions')) {
        ...
        console.warn(`Aviso: No hay permisos para asegurar admin en DB para ${uid}. Esto es normal si el usuario no es el dueño inicial o reglas no propagadas.`);
      }
    }
  }
};
...
export const getReservationCount = async (): Promise<number> => {
  try {
    const q = db.collection(KEYS.TASKS).where('type', '==', TaskType.RESERVATION);
    const snapshot = await q.get();
...
export const clearAllReservations = async () => {
  try {
    ...
    handleFirestoreError(error, OperationType.DELETE, KEYS.TASKS);
  }
};
...
export const saveTask = async (task: Partial<Task>, newFiles: File[] = [], newVideos: File[] = []) => {
  ...
  if (isNew) {
     ...
     // Immediate deletion for unique (non-recurring) tasks, UNLESS it's an announcement
     const isUnique = !fullTask.recurrence || fullTask.recurrence === TaskRecurrence.NONE;
     const isAnnouncement = fullTask.type === TaskType.ANNOUNCEMENT;
     
     if (isUnique && !isAnnouncement) {
